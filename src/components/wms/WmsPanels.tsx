...
export function WmsSlotsPanel({ lang }: { lang: Lang }) {
  ...
  const aisles = [...new Set(slots.map((s) => s.aisle))];
  const filtered = slots.filter(
    (s) => (zone === "all" || s.zone === zone) && (aisle === "all" || s.aisle === aisle),
  );
  const preview = filtered.slice(0, 120);
  ...
  return (
    <div className="space-y-4">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h2 className="font-[family-name:var(--mps-display)] text-2xl text-[var(--ink)]">
            {lang === "es" ? "Mapa de huecos" : "Slot map"}
          </h2>
          <p className="mt-1 text-sm text-[var(--ink-muted)]">
            {lang === "es"
              ? "Cada ubicación tiene dueño: libre, ocupado, reservado o bloqueado."
              : "Every location has an owner: free, occupied, reserved or blocked."}
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            {zones.map((z) => (
              <option key={z} value={z}>
                {ZONE_LABEL[z as keyof typeof ZONE_LABEL]?.[lang] ?? z}
              </option>
            ))}
          </select>
          <select
          ...
          >
            <option value="all">{lang === "es" ? "Todos los pasillos" : "All aisles"}</option>
            {aisles.map((a) => (
      ...
      <div className="grid grid-cols-4 gap-2 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10">
        {preview.map((s) => (
          <button
            key={s.id}
            type="button"
            title={`${s.code} · ${s.status}${s.palletId ? ` · ${s.palletId}` : ""}`}
            className={cn(
              "rounded-lg border px-1.5 py-2 text-center transition hover:scale-[1.03]",
              s.status === "libre" && "border-[color-mix(in_oklab,var(--ok)_40%,transparent)] bg-[color-mix(in_oklab,var(--ok)_12%,transparent)]",
              s.status === "ocupado" && "border-[color-mix(in_oklab,var(--accent)_40%,transparent)] bg-[color-mix(in_oklab,var(--accent)_14%,transparent)]",
              s.status === "reservado" && "border-[color-mix(in_oklab,var(--warn-ink)_40%,transparent)] bg-[var(--warn-bg)]",
              s.status === "bloqueado" && "border-[color-mix(in_oklab,var(--danger)_40%,transparent)] bg-[color-mix(in_oklab,var(--danger)_12%,transparent)]",
              s.status === "inventario" && "border-[var(--glass-border)] bg-[var(--surface-sunken)]",
            )}
          >
            <p className="font-mono text-[10px] font-semibold text-[var(--ink)]">{s.code}</p>
            <p className="mt-0.5 text-[9px] uppercase tracking-wide text-[var(--ink-muted)]">{s.status}</p>
          </button>
        ))}
      </div>
      <p className="text-xs text-[var(--ink-muted)]">
        {lang === "es"
          ? `Mostrando ${preview.length} de ${filtered.length} huecos filtrados · ${slots.length} en el centro.`
          : `Showing ${preview.length} of ${filtered.length} filtered slots · ${slots.length} on site.`}
      </p>
    </div>
  );
}
